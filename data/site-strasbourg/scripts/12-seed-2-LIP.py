...
def extract_seed_ts(seed_name, target_seed, func_file, subject):
    ...
    print(f"\n++ Processing {seed_name} -> {target_seed}")
    print(f"Functional file: {func_file}")
    print(f"Seed mask: {seed_mask}")
    ...
    # Run fslmeants to extract time series
    env = os.environ.copy()
    ...
    if not os.path.exists(ts_file):
        ...
        try:
            ...
            print("++ fslmeants completed successfully")
        except subprocess.CalledProcessError as e:
            print(f"!! fslmeants failed with error: {e.stderr}")
            return None
    ...
    if not os.path.exists(ts_file):
        print("!! ERROR: Time series file not found after fslmeants.")
        return None

    ts = np.loadtxt(ts_file)
    if len(ts) < 2:
        print(f"!! ERROR: Insufficient time points ({len(ts)})")
        return None

    print(f"++ Extracted {len(ts)} time points")
    return ts
...
def compute_correlations(subject, func_file):
    ...
    # Step 1: Extract reference seed (e.g., LIP)
    print("\n=== STEP 1: Extract Reference Time Series ===")
    ...
    print("\n=== STEP 2: Process Seed Connections ===")
    results = {}
...
if __name__ == "__main__":
    ...
    for subject, func_file in subjects_files:
        print(f"\n=== Running analysis for {subject} ===")
        results = compute_correlations(subject, func_file)

        if results:
            print("\n=== Final Results ===")
            for seed, r in results.items():
                print(f"{seed}: {r if r is not None else 'Failed'}")
