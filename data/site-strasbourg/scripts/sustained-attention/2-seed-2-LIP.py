...
SEED_NAMES = ["V1", "V2", "V3", "V4", "V4t", "MT", "MST", "FST", "FEF"]
TARGET_SEED = "LIP"
OUTPUT_DIR = os.path.join(FC_DIR, f"seed-to-{TARGET_SEED}-connectivity")
DERIVATIVES_DIR = os.path.join(FC_DIR, "attention-seed-2-seed-connectivity")
...
def get_subjects_and_sessions():
    subjects = []
    for subject in os.listdir(CLEAN_DIR):
...
def extract_seed_ts(seed_name, target_seed, func_file, subject):
    ts_file = os.path.join(OUTPUT_DIR, f"{subject}_{seed_name}_{target_seed}_ts.txt")
    ...
    if not os.path.exists(ts_file):
        ...
        try:
            ...
            print(f"\n!! fslmeants failed with error: {e.stderr.strip()}")
            return None
    ...
    if not os.path.exists(ts_file):
        print("\n!! ERROR: Time series file not found after fslmeants.")
        return None

    ts = np.loadtxt(ts_file)
    if len(ts) < 2:
        print(f"\n!! ERROR: Insufficient time points ({len(ts)})")
        return None
