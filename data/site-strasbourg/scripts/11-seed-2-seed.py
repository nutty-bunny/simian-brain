...
def extract_seed_ts(subject, seed_name):
    ...
    ts_file = os.path.join(seed_dir, f"{subject}_{seed_name}-seed-ts.txt")
...
def plot_connectivity_matrix(matrix, subject):
    path = os.path.join(OUTPUT_DIR, f"{subject}-seed-connectivity.svg")
...
def generate_group_matrix():
    ...
    mean_df.to_csv(os.path.join(OUTPUT_DIR, "group-mean-connectivity-matrix.csv"))
    print(f"++ Group matrix saved: group-mean-connectivity-matrix.csv")
...
def plot_group_matrix(matrix, significance_mask):
    path = os.path.join(OUTPUT_DIR, "group-mean-connectivity-matrix.svg")
    plt.figure(figsize=(10, 8))
    sns.set(font_scale=1.2, style="white")
